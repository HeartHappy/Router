...
class ClassLoaderServiceImpl : ClassLoaderService {
    ...
    private fun setInjectionValue(extras : Bundle?, paramInfo : InjectParams, field : Field, thiz : Any) {
        val value = extras?.get(paramInfo.name)
        value?.let { result -> //                        logger.debug("inject: ${paramInfo.type}, ${paramInfo.name},  $result")
            when (paramInfo.type) {
                ...
                CharSequence::class -> field.set(thiz, result as? CharSequence)
                Bundle::class -> field.set(thiz, result as? Bundle)
                Array::class -> if (result is Array<*>) field.set(thiz, result)
                ArrayList::class -> field.set(thiz, result as? ArrayList<*>)
                SparseArray::class -> field.set(thiz, result as? SparseArray<*>)
                else -> {
                    if (Parcelable::class.java.isAssignableFrom(paramInfo.type.java)) {
                        field.set(thiz, result as? Parcelable)
                        logger.debug("inject: Parcelable:${paramInfo.type.java.simpleName}")
                    } else if (Serializable::class.java.isAssignableFrom(paramInfo.type.java)) {
                        field.set(thiz, result as? Serializable)
                    } else {
                        field.setObject(result, paramInfo, thiz)
                    }
                }
            }
        } ?: let { //该注入参数没有传递
